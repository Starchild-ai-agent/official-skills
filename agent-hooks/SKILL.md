...
| `verify_code_changes.py` | `pre_tool_call` (recorder) + `on_stop` (decider) | **Anti-"false done" for code.** When you edit a source file but run no test/build/lint to check it, blocks the stop once and steers you to verify (or say plainly there's nothing to run) before finishing. Detects code edits via file tools (`edit_file`/`write_file`) AND common bash writes (heredoc/redirect `> foo.py`, `tee`, `sed -i`) — toggle the latter with `DETECT_BASH_WRITES`. Not exhaustive: a script that writes a file, `cp`/`mv`, awk-rewrites are out of scope (it's a nudge, not a sandbox). Docs/data edits (`.md`/`.json`/`.yaml`/…) are exempt; one nudge per edit-set, self-disarms, fails open. Wire BOTH events to the same script path. |
