...
#
# Flags:
#   --no-footer      skip the two footer hooks (suppress_model_footer + append_runtime_footer)
...
# Production scripts to install (script:has_selftest)
SCRIPTS=(security_guard verify_publish_claims)
if [ "$WANT_FOOTER" = 1 ]; then
  SCRIPTS+=(suppress_model_footer append_runtime_footer)
fi
...
YAML
  ...
  # ── Footer policy (two hooks, used together) ──────────────────────
  - event: pre_llm_call
