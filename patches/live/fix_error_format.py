...
def _classify(raw_str: str, symbol: str) -> tuple:
    ...
    if "api_key" in raw_lower or "api key" in raw_lower or "401" in raw_str or "403" in raw_str:
        ...
        return "auth", "Authentication issue with upstream API", "This is managed by the platform. Retry or contact support."
    ...
    if "not found" in raw_lower or "invalid" in raw_lower:
        return "invalid_input", f"Invalid parameter", f"Check symbol/parameters. Symbol: '{symbol}'"
...
if __name__ == "__main__":
    ...
    print(f"✅ BUG-5 test 4: Non-error passed through unchanged")
