...
class CryptoWorkflowTester:
    ...
    def _check_tx_lifecycle(self, skill, files):
        ...
        sends_tx = bool(
            re.search(
                r'(?:send_transaction|broadcast|submit|swap|transfer|order|sign.*send)',
                combined,
                re.I))
        if not sends_tx:
            return
        ...
        has_receipt_check = bool(
            re.search(
                r'(?:receipt|get_transaction|wait.*confirm|poll|finality|block_number)',
                combined,
                re.I))
        ...
        if not has_receipt_check:
            self._add(
                ...
                'HIGH',
                'NO_TX_CONFIRMATION',
                'Skill sends transactions but never checks receipt/'
                'confirmation. Agent says "done" before on-chain finality.',
    ...
    def _check_gas_handling(self, skill, files):
        ...
        has_gas_estimate = bool(
            re.search(
                r'(?:estimate_gas|gasLimit|gas_limit|gas_price|maxFeePerGas)',
                combined,
                re.I))
    ...
    def _check_slippage_handling(self, skill, files):
        ...
        has_slippage = bool(
            re.search(
                r'(?:slippage|slip|min_amount|minReturn|minimum.*out|amountOutMin)',
                combined,
                re.I))
    ...
    def _check_chain_id_handling(self, skill, files):
        ...
        has_chain_validation = bool(
            re.search(
                r'(?:supported.*chain|valid.*chain|chain.*(?:not|in|supported))',
                combined,
                re.I))
    ...
    def _check_rate_limit_awareness(self, skill, files):
        ...
        has_rate_handling = bool(
            re.search(
                r'(?:rate.?limit|429|too.?many|throttl|sleep|delay|backoff)',
                combined,
                re.I))
    ...
    def _check_amount_precision(self, skill, files):
        ...
        has_precision_handling = bool(
            re.search(
                r'(?:10\s*\*\*|decimals|Decimal|int\(|float\(|wei.*ether|from_wei|to_wei)', combined))
    ...
    def _check_address_validation(self, skill, files):
        ...
        has_validation = bool(
            re.search(
                r'(?:is_address|checksum|0x[a-fA-F0-9]{40}|isAddress|is_valid_address|len\(.*\)\s*(?:!=|==)\s*4[02])',
                combined))
