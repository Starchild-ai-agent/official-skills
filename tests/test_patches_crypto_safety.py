...
from shared.crypto_safety import (
    ...
    CHAIN_FINALITY, L2_WITHDRAWAL_CHALLENGE, DEFAULT_SLIPPAGE, GAS_ESTIMATES
)
...
test("get_finality_info returns correct data for ethereum", t_finality_known)
...
test("get_finality_info returns safe defaults for unknown chain", t_finality_unknown)
...
test("All 8 major chains have finality data", t_finality_all_chains)
...
test("L2s have 7-day challenge periods", t_l2_challenges)
...
def t_finality_message():
    ...
    assert "3 min" in msg
test("format_finality_message includes emoji, truncated hash, explorer, time", t_finality_message)
...
test("L2 finality message warns about withdrawal challenge period", t_finality_l2_warning)
...
test("Gas estimate: eth_transfer = 21000 units + 20% buffer", t_gas_known)
...
test("Gas estimate: unknown operation gets 200K default", t_gas_unknown)

# D9: Gas estimates cover common operations
def t_gas_coverage():
    for op in ["eth_transfer", "erc20_transfer", "erc20_approve", "swap_simple", "swap_complex", "aave_deposit", "aave_withdraw"]:
        assert op in GAS_ESTIMATES, f"{op} should have gas estimate"
test("Gas estimates cover 7 common operations", t_gas_coverage)
...
test("Slippage: USDC/USDT = 0.1% (stablecoin)", t_slippage_stable)
...
test("Slippage: ETH/USDC = 0.5% (major pair)", t_slippage_major)
...
test("Slippage: uses 24h volume to classify liquidity", t_slippage_volume)
...
test("Slippage: unknown tokens get 1% default", t_slippage_default)
...
test("Slippage suggestion includes readable message", t_slippage_message)
...
test("Verification checklist includes operation, chain, tx hash", t_checklist_basic)
...
test("Verification checklist includes expected balance/position changes", t_checklist_expected)
