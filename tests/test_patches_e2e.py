...
from shared.errors import (
    SkillError, RateLimitError, InsufficientBalanceError,
    SlippageExceededError, safe_call
)
from shared.response import ok, fail, fmt_price, fmt_balance
from shared.retry import RetryConfig, with_retry, retry_api_call
from shared.crypto_safety import (
    get_finality_info, suggest_slippage, estimate_gas_needed,
    verification_checklist, format_finality_message
)
...
def t_multichain_balance():
    ...
    for chain in chains:
        try:
            ...
            results[chain] = fail(
                f"wallet/balance", f"Failed to query {chain}",
