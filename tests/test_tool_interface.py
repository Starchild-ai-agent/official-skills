...
class ToolInterfaceTester:
        ...
        for m in param_blocks:
            ...
            if required_count > 4:
                self._add(
                    ...
                    line_no,
                    'HIGH',
                    'TOO_MANY_REQUIRED_PARAMS',
                    f'Tool "{class_name}" has {required_count} required params. '
                    'Small model error rate rises sharply above 3.',
    ...
    def _check_param_validation(self, skill, fname, content):
        ...
        for m in re.finditer(r'async\s+def\s+execute\s*\(self.*?\).*?:', content):
            ...
            has_validation = bool(
                re.search(
                    r'(?:if\s+not\s+|if\s+\w+\s*(?:is None|==|!=|not in)|raise\s+\w*(?:Value|Type|Tool)Error|validate)',
                    body))
    ...
    def _check_param_defaults(self, skill, fname, content):
        ...
        for m in re.finditer(
            r'ToolParameter\s*\(\s*name\s*=\s*["\'](\w+)["\'].*?required\s*=\s*False',
            content,
                re.DOTALL):
